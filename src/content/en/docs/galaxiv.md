---
title: "Galaxiv"
description: "Galaxiv is web-based strategy game that teaches users on the starmap and basic principles of Supereconomics"
date: 2022-01-18T19:58:14+01:00
lastmod: 2022-01-18T19:58:14+01:00
image: "/og/build.jpg"
---


Galaxiv is web-based strategy game that teaches users on the starmap and the first 3 laws of Supereconomics:

1. Value is based on Relational Desire
2. Value is created to remove lack
3. There must be balance in the creation and spread of value
4. Value is spread through fair exchange

